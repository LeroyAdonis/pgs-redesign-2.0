  ...
  return (
    <div className={cn("space-y-3", className)}>
      {Array.from({ length: lines }, (_, i) => (
        <div
          key={i}
          className="relative overflow-hidden rounded-none bg-surface-inset"
...
export function StatCardShimmer() {
  return (
    <div className="overflow-hidden rounded-none border border-border bg-surface-raised p-5">
      <div className="relative mb-3 h-9 w-9 overflow-hidden rounded-none bg-surface-inset">
        <div
          className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/10 to-transparent"
          style={{ animation: "shimmer 1.5s infinite" }}
        />
    ...
    <div className="flex flex-col items-center justify-center gap-3 py-8 text-center">
      ...
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="rounded-none bg-brand-surface px-3 py-1.5 text-xs font-medium text-brand transition-colors hover:bg-brand/20"
        >
          Try again
        </button>
      )}
