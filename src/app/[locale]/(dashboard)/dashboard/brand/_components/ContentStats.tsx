...
export function ContentStats({ avgContentLength, visualStyle }: ContentStatsProps) {
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div className="rounded-none bg-surface-inset p-4">
      ...
      <div className="rounded-none bg-surface-inset p-4">
        ...
        <div className="mt-3 flex gap-1.5">
          {visualStyle.colorPalette.map((color, i) => (
            <div
              key={`${color}-${i}`}
              className="h-10 flex-1 rounded-none shadow-sm"
            ...
            <div key={type} className="flex items-center gap-2">
              <span
                className={cn(
                  ...
                  'rounded-none bg-brand-surface text-sm',
                )}
              >
          ...
          <>
            ...
            <div className="mt-2 flex flex-wrap gap-1">
              {visualStyle.filterPreferences.map((filter) => (
                <span
                  key={filter}
                  className="rounded-none bg-brand-surface px-2 py-0.5 text-xs text-brand"
                >
                  {filter}
                </span>
              ))}
